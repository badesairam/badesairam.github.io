...
A policy rules how the agent selects the action to perform given the state \\( a_{t} \sim \pi(.|s_{t})\\). The aim is to identify an optimal policy that maximizes the expected reward J given policy \(\pi_\theta\), parameterized by parameters \(\theta$\):
